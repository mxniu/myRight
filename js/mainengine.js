	...
	function setupFactors()
	{
		...
		for(x in fstack)
		{
			totalWidth += $('li[id="factor_' + fstack[x]['factor'] + '"]').width();
		}
	...
	$.fn.animateFactor = function(FACTORNAME)
	{
		...
		else
		{
			...
			$("li[id^='factor_']").click(function() {});
						...
						$("li[id^='factor_']").hover(function() {
							...
							if(this_factor != current_factor)
								$("li[id^='factor_"+this_factor+"']").children().css('background-position', function(i,v){
									return v.replace(/-?\d+px$/, '-34px');
								});
						}, function() {
							var this_factor = $(this).attr("id").split("_")[1];
							if(this_factor != current_factor)
			...
			$("li[id^='factor_']").children().css('background-position', function(i,v){
				return v.replace(/-?\d+px$/, '0px');
			});
			$("li[id^='factor_"+FACTORNAME+"']").children().css('background-position', function(i,v){
					return v.replace(/-?\d+px$/, '-34px');
			});
			
			if(factor_text_color)
			{
				$("li[id^='factor_']").css('color', "#000");
				$("li[id^='factor_"+FACTORNAME+"']").css('color', factor_text_color);
			}
